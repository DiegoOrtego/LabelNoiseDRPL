...
def get_dataset(args, transform_train, transform_test, num_classes):
    ...
    #################################### Train set #############################################
    cifar_train = Cifar10Train(args, train=True, transform=transform_train, download=args.download, pslab_transform = transform_test)
...
class Cifar10Train(tv.datasets.CIFAR10):
    ...
    def real_in_noise(self):
        ...
        for i in range(len(idxes)):
            self.soft_labels[idxes[i]][self.train_labels[idxes[i]]] = 0  ## Remove soft-label created during label mapping
            current_label = self.train_labels[idxes[i]]
            if self._num > 0:
                # current_label = self.train_labels[idxes[i]]
